...
- Tests need Node >= 22 (`.ts` type stripping). System `/usr/local/bin/node` is v20.
  ...
  wrapper — ticket `docs/tickets/0003-broken-node-nvm-shell-profile.md`).
- Fix on a fresh sandbox (wipes `/home/node/.nvm`): install the Node v26.4.0 **aarch64**
  tarball to `/home/node/.nvm/versions/node/v26.4.0/` and write the minimal
