...
**Addendum (2026-07-13) — always-visible magnifier:** the magnifier was originally visible ONLY in zen; it is now ALWAYS visible and the cluster's LEFTMOST icon. The plugin's shared Flex wrapper is dissolved (`display: contents`) so the magnifier is its own flex item ordered first (`order: -1`) while the lotus keeps its next-to-fullscreen slot; the zen hide rule moved from wrapper-content to wrappers (`:has()`) so emptied wrappers leave no flex-gap holes mid-cluster. The magnifier also survives READER mode (custom.scss exempts the zen wrapper from the reader wrapper-hide, hides only the lotus, and forces the dimmed `.search` root opaque while the overlay is open).
**Owner:** Nickolay
